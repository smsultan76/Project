...
<body>
    ...
    <div id="classh2">
        <h2>Here you can show and manage CE Department Student information</h2>
    </div>
    <?php
        ...
        <div id="addstudent">
            ...
            if (isset($_POST["submit"])) {
                ...
                if ($_SERVER["REQUEST_METHOD"] == "POST") {
                    ...
                    $addate = $_POST["addate"];
                    $sql = "SELECT * FROM ce WHERE roll ='$roll' OR regn ='$regn'";
                    if (mysqli_fetch_assoc(mysqli_query($conn, $sql))) {
                        submsg("Faild! Roll or Registration number already exist.", "red");
                    } else {
                        try {
                            $sql = "INSERT INTO ce(roll, firstname, lastname, fname, mname,	regn, phone, email,	`address`, `session`,addate)
    ...
    if (isset($_POST["rsubmit"])) {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            $roll = $_POST["rroll"];
            $reg = $_POST["rregn"];
            $sql = "SELECT * FROM `ce` WHERE roll=$roll && regn=$reg";
            try {
                if (mysqli_fetch_assoc(mysqli_query($conn, $sql))) {
                    $sql2 = "DELETE FROM `ce` WHERE roll=$roll";
    ...
    <div id="Edit">
        <div id="editinfo">
            ...
            if (isset($_POST["esubmit"])) {
                $eroll = $_POST["eroll"];
                $sql = "SELECT * FROM ce WHERE roll='$eroll'";
                if (mysqli_num_rows(mysqli_query($conn, $sql)) > 0) {
                    $result = mysqli_fetch_assoc(mysqli_query($conn, $sql));
                        session_start();
                        $_SESSION["stdinfo"] = $result;
                        header("Location: edit/ceedit.php");
                        exit();
